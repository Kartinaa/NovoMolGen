...
This version automatically detects whether to use Tanimoto or InfoNCE model based on config.

Usage:
    python scripts/generate_from_full_model_auto.py \
...
def detect_model_type(model_path: Path, logger: logging.Logger):
    """
    Detect model type by reading config.json and checking tanimoto_weight.
...
def load_model_and_tokenizer(model_path: str, logger: logging.Logger):
    """Load model and tokenizer from full_model directory with automatic model type detection."""
    ...
    if hasattr(config, "tanimoto_weight"):
        logger.info(f"  tanimoto_weight={config.tanimoto_weight}")
...
def main():
    parser = argparse.ArgumentParser(description="Generate molecules from trained model for validation sets (auto-detect model type)")
    ...
    logger.info("=" * 60)
    logger.info("Molecule Generation from Trained Model (Auto-Detect)")
    logger.info("=" * 60)
