...
# parents[3] because file is at scripts/data_processing/molgenbench/04_compute_ligand_vec.py
...
def load_dataset(input_file: Path):
    """Load the dataset and normalise ligand_path to relative paths."""
    print(f"Loading dataset from {input_file}")
    df = pd.read_parquet(input_file)
    print(f"Loaded {len(df)} samples")

    if "ligand_path" in df.columns:
        df["ligand_path"] = df["ligand_path"].apply(to_relative_path)
        print(f"Normalised ligand_path to relative (e.g. {df['ligand_path'].iloc[0]})")
...
def compute_ligand_vectors(df: pd.DataFrame, output_dir: Path, batch_size: int = 32):
    ...
    try:
        ...
        for ligand_path in tqdm(df['ligand_path'], desc="Converting SDF to SMILES"):
            abs_path = to_absolute_path(ligand_path)
            smiles = sdf_to_smiles(abs_path)
            if smiles is None:
