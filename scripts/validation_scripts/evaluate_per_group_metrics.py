...
def evaluate_per_group_metrics(
    ...
    for g in range(num_groups):
        ...
        # 计算该组生成分子与对应文件夹中配体的 Tanimoto 相似度（如果提供了 folder list）
        if validation_folder_list is not None and valid_cnt > 0 and (folder_paths or idx_to_folder):
            ...
            if lig_folder is not None:
                ...
                if lig_path.exists():
                    ...
                    if lig_mol is not None:
                        ...
                        logger.debug(f"Failed to read ligand from {lig_path}")
                else:
                    logger.debug(f"Ligand file not found: {lig_path}")
