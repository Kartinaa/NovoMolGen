#!/usr/bin/env python3
"""
测试条件是否真正被模型使用（支持自动检测 Tanimoto/InfoNCE 模型）：
...
用法示例：
  python scripts/validation_scripts/test_condition_effect_auto.py \
...
def load_model_and_tokenizer(model_path: str, logger: logging.Logger):
    """与 generate_from_full_model.py 一致的加载方式，支持自动检测模型类型。"""
...
def main():
    parser = argparse.ArgumentParser(
        ...
        "by comparing correct vs shuffled conditions (auto-detect model type)."
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    logger.info("=" * 60)
    logger.info("Test Condition Effect via NLL Loss (Auto-Detect Model Type)")
    logger.info("=" * 60)
