...
# RDKit imports for Tanimoto similarity computation
try:
    ...
    logger.warning("RDKit not available. Tanimoto loss will not work.")
...
# ============================================================================
# Tanimoto Loss Utility Functions
# ============================================================================
...
def compute_morgan_fingerprint(smiles: str, radius: int = 2, n_bits: int = 2048):
    ...
    Returns:
        RDKit fingerprint object, or None if molecule is invalid
    """
...
def compute_tanimoto_matrix(smiles_list: List[str], radius: int = 2, n_bits: int = 2048):
    """
    Compute Tanimoto similarity matrix for a list of SMILES.
    ...
    Returns:
        Tuple of (similarity_matrix [B, B], valid_mask [B])
        - similarity_matrix: Tanimoto similarity between molecules
        - valid_mask: Boolean mask indicating valid molecules
    """
    if not RDKIT_AVAILABLE:
        B = len(smiles_list)
        return torch.zeros(B, B), torch.zeros(B, dtype=torch.bool)
    ...
    for smi in smiles_list:
        fp = compute_morgan_fingerprint(smi, radius, n_bits)
        fps.append(fp)
...
def compute_euclidean_distance_matrix(z: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    """
    # z: [B, z_dim]
    # Compute ||z_i - z_j||^2 = ||z_i||^2 + ||z_j||^2 - 2 * z_i . z_j
...
def compute_tanimoto_loss(
    ...
    """
    Compute Tanimoto loss to encourage similar molecules to have similar z vectors.

    L_Tanimoto = 1 + rho(T, D)

    Where:
    - T = N×N Tanimoto similarity matrix
    - D = N×N Euclidean distance matrix of z vectors
    - rho = Pearson correlation over upper-triangular entries
    - Loss = 0 when rho = -1 (high similarity <-> low distance)

    Args:
        z: Latent vectors [B, z_dim]
        smiles_list: List of SMILES strings (length B)
    ...
    if not RDKIT_AVAILABLE:
        logger.warning("RDKit not available, returning 0 Tanimoto loss")
        return torch.tensor(0.0, device=device, requires_grad=True)

    # Compute Tanimoto similarity matrix T [B, B] (on CPU, then move to device)
    T, valid_mask = compute_tanimoto_matrix(smiles_list, fp_radius, fp_bits)
    T = T.to(device)
...
class NovoMolGenConfig(LlamaConfig):
    ...
    def __init__(self,
        ...
        self.auto_map = {"AutoModelForCausalLM": "modeling_novomolgen_tanimoto.NovoMolGen"}
...
class NovoMolGen(GPTLMHeadModel):
    ...
    def forward(self, input_ids, attention_mask: Optional[torch.FloatTensor] = None,
        ...
        # Add Tanimoto loss if enabled
        if tanimoto_loss is not None and tanimoto_weight > 0.0:
            loss = loss + tanimoto_weight * tanimoto_loss if loss is not None else tanimoto_weight * tanimoto_loss
        ...
        if self.training:
            ...
            self._last_tanimoto_loss = tanimoto_loss.item() if tanimoto_loss is not None else 0.0
